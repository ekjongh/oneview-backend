...
from app.crud.rrc import get_rrc_trend_by_group_date2,get_worst10_rrc_bts_by_group_date2,get_rrc_trend_item_by_group_date
...
async def get_rrc_trend_day2(code:str=None, group:str="", start_date: str = "20220810", end_date: str = None, db: SessionLocal = Depends(get_db)):
    rrc_trend_days = await get_rrc_trend_by_group_date2(db=db, code=code, group=group, start_date=start_date, end_date=end_date)
    return rrc_trend_days


@router.get("/worst2", response_model=List[schemas.RrcBtsOutput])
async def get_worst_rrc_bts2(limit: int = 10, code:str=None, group:str="", prod:str="RRC성공율", start_date: str = "20220821", end_date: str = None, db: SessionLocal = Depends(get_db)):
